$(function() {
	// 获取此店铺下的商品列表的URL
	var listUrl = '/foodShop/shopadmin/getproductlistbyshop?pageIndex=1&pageSize=999';
	// 商品下架URL
	var statusUrl = '/foodShop/shopadmin/modifyproduct';
	getList();
	/**
	 * 获取此店铺下的商品列表
	 * 
			...
			.on(
					...
					function(e) {
						...
						if (target.hasClass('edit')) {
							...
							window.location.href = '/foodShop/shopadmin/productoperation?productId='
							...
							window.location.href = '/foodShop/frontend/productdetail?productId='
									+ e.currentTarget.dataset.id;
						}
					});
	function changeItemStatus(id, enableStatus) {
		...
		$.confirm('确定么?', function() {     //出现 是否确定  的确认框
			// 上下架相关商品
			$.ajax({
				url : statusUrl,
				type : 'POST',
