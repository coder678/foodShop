$(function() {
	...
	var pageSize = 3;
	// 获取店铺列表的URL
	var listUrl = '/foodShop/frontend/listshops';
	// 获取店铺类别列表以及区域列表的URL
	var searchDivUrl = '/foodShop/frontend/listshopspageinfo';
	// 页码
	var pageNum = 1;
	...
	$(document).on('infinite', '.infinite-scroll-bottom', function() {
		if (loading)
			return;
		addItems(pageSize, pageNum);    //执行 加载店铺卡片的操作
	});

	// 点击店铺的卡片进入该店铺的详情页  通过拼接店铺地址的url
	$('.shop-list').on('click', '.card', function(e) {
		var shopId = e.currentTarget.dataset.shopId;
		window.location.href = '/foodShop/frontend/shopdetail?shopId=' + shopId;
	});
	...
	$('#shoplist-search-div').on(
			...
			function(e) {
				if (parentId) {// 如果传递过来的是一个父类下的子类
					...
					//若选定  新的类别， 那么就首先 消除掉之前的选中按钮的效果，然后重置新按钮的样式
