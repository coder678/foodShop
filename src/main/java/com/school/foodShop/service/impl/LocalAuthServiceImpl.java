...
public class LocalAuthServiceImpl implements LocalAuthService {
	...
	@Override
	public LocalAuthExecution insertAuth(LocalAuth localAuth) throws LocalAuthOperationException {
		if (localAuth.getUsername() != null && localAuth.getPassword() != null) {
		    if (localAuthDao.queryLocalByUserNameAndPwd(localAuth.getUsername(),localAuth.getPassword()) == null) {
				localAuthDao.insertLocalAuth(localAuth);
				return new LocalAuthExecution(LocalAuthStateEnum.SUCCESS,localAuth);
		    }
		    return new LocalAuthExecution(LocalAuthStateEnum.ONLY_ONE_ACCOUNT,localAuth);
		}
		return new LocalAuthExecution(LocalAuthStateEnum.NULL_AUTH_INFO,localAuth);
	}
